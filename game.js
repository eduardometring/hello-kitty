...
playerImg.src = 'images/hello_kitty.png';
...
['leftBtn','rightBtn','upBtn'].forEach(id=>{
  ...
  btn.addEventListener('pointerdown', e=>{ e.preventDefault(); touchKeys[keyMap]=true; });
  btn.addEventListener('pointerup', e=>{ e.preventDefault(); touchKeys[keyMap]=false; });
  btn.addEventListener('pointerleave', e=>{ touchKeys[keyMap]=false; });
});
...
function draw(){
  ...
  const cameraX = Math.max(0, Math.min(player.x-canvas.width/2, worldWidth-canvas.width));
  ...
  for(const c of collectibles){
    if(!c.collected){
      ctx.fillStyle='#ff3b3b'; ctx.beginPath(); ctx.arc(c.x,c.y,c.r,0,Math.PI*2); ctx.fill();
      ctx.fillStyle='#3aa655'; ctx.fillRect(c.x-2,c.y-c.r-4,4,6);
    }
  }
  ...
  ctx.fillText(`🍓 ${score} / ${collectibles.length}`,12,24);
  ...
  if(gameWon){
    ...
    ctx.fillText('💖 Fim de fase! 💖',canvas.width/2,canvas.height/2-20);
    ctx.font='18px Arial';
    ctx.fillText(`Você coletou ${score} morangos 🍓`,canvas.width/2,canvas.height/2+15);
  }
